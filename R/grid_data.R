...
#' @param map controls whether a map should be computed while running \code{grid_data}. It is
...
grid_data <- function(lat, lon, myresolution = 0.5, myzoom = 7, lat_centre = NULL, lon_centre = NULL, map = FALSE){
  ...
  raw_data <- data.frame(lat, lon, mycoord)
  ...
  return(list(raw_data = raw_data, gridded_data = dat, myplot = p))
}